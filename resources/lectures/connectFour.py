...
#
#              There were 5 SYNTACTIC ERRORS (mistakes
...
#
#              Your job was to find (and correct!) each
#              of these mistakes using your new
#              DEBUGGING TECHNIQUES.
# ----------------------------------------------------
# SYNTAX ERROR 1: forget colon
def printWelcome():
    print("+----------------+")
    print("|   welcome to   |")
    print("|  CONNECT FOUR  |")
...
def getBoardSize():
    ...
    # SYNTAX ERROR 2: forget to capitalize True
    # while true:
    while True:
        try:
            # SYNTAX ERROR 3: forget to cast input to int
            # n = input('Please enter a board size between 1 and 10: ')
            n = int(input('Please enter a board size between 1 and 10: '))
        except ValueError:
...
def play():
    ...
    # Print the board
    print('Current board:')
    # LOGICAL ERROR 1: print list instead of each item
    # print(grids)
    for row in grids:
        print(row)
    ...
    while True:
        ...
        print('\nCurrent board:')
        # LOGICAL ERROR 2: print list instead of each item
        # print(grids)
        for row in grids:
            print(row)

        if (check_horizontal_win(grids, player, n) or
            # LOGICAL ERROR 3: forget to check vertical wins
            check_vertical_win(grids, player, n) or
        ...
        # LOGICAL ERROR 4: forget to switch players
        if (player == 1):
            player = 2
        else:
            player = 1
...
# SYNTAX ERROR 5: forget def
# check_vertical_win(grids, player, n):
def check_vertical_win(grids, player, n):
...
# LOGICAL ERROR 5: forget to call main()
main()
